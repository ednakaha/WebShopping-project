...
export class RegisterService {
  ...
  addPersonStep1(Person: PersonM) {
    //debugger;
    this.http.post<PersonM>(environment.url + '/register/addStep1', Person, httpOptions).subscribe(
      data => {
  ...
  addPersonStep2(Person: PersonM) {
    //debugger;
    this.http.post<PersonM>(environment.url + '/register/addStep2', Person, httpOptions).subscribe(
      data => {
